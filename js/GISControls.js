...
THREE.GISControls = function ( object, domElement ) {
	...
	this.panLeft = function ( distance ) {
            pan.x += distance * Math.sin( theta * Math.PI / 180 );
            pan.y -= distance * Math.cos( theta * Math.PI / 180 );
	};

	// pass in distance in world space to move up
	this.panUp = function ( distance ) {
            pan.x -= distance * Math.cos( theta * Math.PI / 180 );
            pan.y -= distance * Math.sin( theta * Math.PI / 180 );
        };
	...
	function onMouseMove( event ) {
		...
		if ( state === STATE.ROTATE ) {
			...
			theta -= 360 * rotateDelta.x / element.clientWidth * scope.rotateSpeed;
			phi += 360 * rotateDelta.y / element.clientHeight * scope.rotateSpeed;
